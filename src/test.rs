...
pub fn event_player_input_gamepad(
    mut query: Query<(&Player, &mut Transform, &GamepadInput)>,
    time: Res<Time>,
    mut gamepad_evr: EventReader<GamepadEvent>,
) {
    for (player, transform, gamepad) in &mut query {
        for ev in gamepad_evr.iter() {
            if let GamepadEvent::Axis(info) = &ev {
                info!("{:?}", info);
            }
        }
    }
